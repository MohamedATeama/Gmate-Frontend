...
const ProjectsDashboard: React.FC = () => {
  ...
  const projectStats = [
    ...
    { label: "Active", value: projects?.filter(p => p.status === 'active').length || 0, icon: <Clock size={16} />, color: "text-emerald-500" },
    { label: "Completed", value: projects?.filter(p => p.status === 'completed').length || 0, icon: <CheckCircle2 size={16} />, color: "text-slate-400" },
